...
fn main() {
    ...
    let threads = 8;
    let rows_per_band = bounds.0 / threads + 1;

    {
         let bands: Vec<&mut [u8]> = pixels.chunks_mut(rows_per_band * bounds.0).collect();
         crossbeam::scope(|spawner| {   // || {} is the form of a closure expression. Return types are inferred by compiler
    ...
    -> Complex<f64>
{
    ...
    Complex {
        ...
        // but the imaginary componant increases as we go up.
    }
}
...
/// Render a rectangle of the Mandelbrot set into a buffer of pixels.assert_eq!
/// 
